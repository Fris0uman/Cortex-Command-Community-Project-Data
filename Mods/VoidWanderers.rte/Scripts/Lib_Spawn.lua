...
function CF_MakeBrainWithPreset(c, p, team, pos, preset, class, module)
	...
	local actor = module and CF_MakeActor(preset, class, module) or CF_MakeActor2(preset, class)
...
function CF_MakeListOfMostPowerfulWeapons(c, p, wtype, tech)
	...
	if #weaps == 0 then
		for i = 0, #CF_WeaponTypes - 1 do
			weaps = CF_MakeListOfMostPowerfulWeapons(c, p, i, tech)
			if weaps then
				break
			end
		end
...
function CF_MakeListOfMostPowerfulActors(c, p, atype, tech)
	...
	if #acts == 0 then
		for i = 0, #CF_ActorTypes - 1 do
			acts = CF_MakeListOfMostPowerfulActors(c, p, i, tech)
			if acts then
				break
			end
		end
	...
	if MovableMan:GetMOIDCount() < CF_MOIDLimit then
		...
		if a ~= nil then
			...
			if reputation then
				...
				setXP = math.max(reputation * 0.2, setXP)
			end
			
			actor = CF_MakeActor(CF_ActPresets[f][a], CF_ActClasses[f][a], CF_ActModules[f][a], math.min(math.floor(RangeRand(setXP * 0.1, setXP) + 0.5), CF_Ranks[#CF_Ranks]))
			...
			if actor and CF_ActClasses[f][a] ~= "ACrab" then
				local chance = setXP/CF_Ranks[#CF_Ranks]
				if math.random() + 0.5 < chance then
