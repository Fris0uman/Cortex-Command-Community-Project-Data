...
function VoidWanderers:MissionCreate()
	...
	for i = 1, #miners do
		...
		if math.random() < 0.5 then
			...
			name = name .. CF_RandomNames[2][math.random(#CF_RandomNames[2])] .. " " .. CF_RandomNames[3][math.random(#CF_RandomNames[3])]
			nw["Name"] = name
		end
	...
	if self.MissionStage == self.MissionStages.ACTIVE then
		...
		for actor in MovableMan.Actors do
			if actor.Team == CF_PlayerTeam then
				...
				if (actor.ClassName == "ACDropShip" or actor.ClassName == "ACRocket") and actor:IsPlayerControlled() then
	...
	if #self.MissionLZs > 0 and self.Time >= self.MissionLastReinforcements + self.MissionSettings["Interval"] then
		if MovableMan:GetMOIDCount() < CF_MOIDLimit then
			...
			if ship then
				...
				for i = 1, count do
					local actor = CF_SpawnAIUnitWithPreset(self.GS, self.MissionTargetPlayer, CF_CPUTeam, nil, Actor.AIMODE_SENTRY, math.random(CF_PresetTypes.HEAVY2))
					if actor then
						ship:AddInventoryItem(actor)
					end
				end
