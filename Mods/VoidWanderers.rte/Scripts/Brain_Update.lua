			...
			if self.healTimer:IsPastSimTimeLimit() then
				...
				for _, healTarget in pairs(self.healTargets) do
					if healTarget and IsActor(healTarget) and (healTarget.Health < healTarget.MaxHealth or healTarget.WoundCount > 0) and healTarget.Vel.Largest < 10 then
						...
						if (trace.Magnitude - healTarget.Radius) < healRange and SceneMan:CastObstacleRay(self.Pos, trace, Vector(), Vector(), self.ThisActor.ID, self.ThisActor.IgnoresWhichTeam, rte.grassID, 5) < 0 then
				...
				self.healTargets = {};
				for actor in MovableMan.Actors do
					if actor.Team == self.ThisActor.Team and actor.ID ~= self.ThisActor.ID and (actor.Health < actor.MaxHealth or actor.WoundCount > 0) and actor.Vel.Largest < 5 then
						local trace = SceneMan:ShortestDistance(self.Pos, actor.Pos, false);
						if (trace.Magnitude - actor.Radius) < (healRange * 0.9) then
							if SceneMan:CastObstacleRay(self.Pos, trace, Vector(), Vector(), self.ThisActor.ID, self.ThisActor.IgnoresWhichTeam, rte.airID, 3) < 0 then
...
function rpgbrain_skill_healend(self)
	if self.HealTarget ~= nil and MovableMan:IsActor(self.HealTarget) and self.HealTarget.Health > 0 then
		...
		local actor = CF_MakeActor(preset, self.HealTarget.ClassName, self.HealTarget.ModuleName, self.HealTarget:GetNumberValue("VW_XP"), self.HealTarget:GetNumberValue("Identity"), self.HealTarget:GetNumberValue("VW_Prestige"), self.HealTarget:GetStringValue("VW_Name"), {})
		if actor then
