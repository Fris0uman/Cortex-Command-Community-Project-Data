			...
			if self.ShipControlMode == self.ShipControlPanelModes.LOCATION then 
				...
				if playable then
					...
					if self.ShipControlLocationList[ self.ShipControlSelectedLocation ] ~= nil then
						local rev = self.GS[self.ShipControlLocationList[ self.ShipControlSelectedLocation ].."-FogRevealPercentage"] or 0
					...
					if gold ~= nil then
						local s = gold == true and "PRESENT" or "ABSENT"
					...
					if cont:IsState(Controller.PRESS_DOWN) then
						...
						for item in MovableMan.Items do
							...
							if  count < tonumber(self.GS["Player0VesselStorageCapacity"]) then
								CF_PutItemToStorageArray(self.StorageItems, item.PresetName, item.ClassName, item.ModuleName)
							else
								break
							end
						end
			...
			if self.ShipControlMode == self.ShipControlPanelModes.MISSIONS then
				...
				if cont:IsState(Controller.WEAPON_FIRE) then
					if not self.FirePressed then
						...
						self.GS["Distance"] = CF_Dist(Vector(tonumber(self.GS["ShipX"]),tonumber(self.GS["ShipY"])), Vector(tonumber(self.GS["DestX"]),tonumber(self.GS["DestY"])))
...
---------------------------------------------------------------------------------------------------
			if self.ShipControlMode == self.ShipControlPanelModes.BRAIN then
				...
				self.ShipControlSkillUpgrades[nm]["Variable"] = "Brain"..plr.."Toughness"
				...
				self.ShipControlSkillUpgrades[nm]["Description"] = "The strength of automatic healing of nearby allies."
				local val = tonumber(self.GS[ self.ShipControlSkillUpgrades[nm]["Variable"] ])
				self.GS[ self.ShipControlSkillUpgrades[nm]["Variable"] ] = val
				self.ShipControlSkillUpgrades[nm]["Price"] = val + 1
