	...
	for plr = 0 , self.PlayerCount - 1 do
		...
		if act and MovableMan:IsActor(act) and act.PresetName == "Clones Control Panel" then
			...
			if self.ClonesControlMode == self.ClonesControlPanelModes.CLONES or self.ClonesControlMode == self.ClonesControlPanelModes.SELL then
				...
				for i = self.ClonesControlCloneListStart, self.ClonesControlCloneListStart + self.ClonesControlPanelLinesPerPage - 1 do
					if i <= #self.Clones and i > 0 then
						...
						local name = (self.Clones[i]["Prestige"] and self.Clones[i]["Name"] and self.Clones[i]["Name"] ~= "") and self.Clones[i]["Name"] or self.Clones[i]["Preset"]
						if i == self.SelectedClone then
							CF_DrawString("> "..name, pos + Vector(-130,-40) + Vector(0, (loc) * 12), 120, 10)
				...
				if self.SelectedClone ~= nil and self.SelectedClone > 0 then
					...
					-- Print inventory
					CF_DrawString(info .. "Inventory: "..#self.Clones[self.SelectedClone]["Items"].."/"..CF_MaxItems, pos + Vector(12,-60) , 300, 10)
								...
								if self.SelectedClone ~= 0 then
									if MovableMan:GetMOIDCount() < CF_MOIDLimit then
										...
										for j = 1, #CF_LimbID do
											limbData[j] = self.Clones[self.SelectedClone][CF_LimbID[j]];
											if not CF_PermanentLimbLoss and limbData[j] == "Null" then
												limbData[j] = nil;
											end
										end
										local a = CF_MakeActor(self.Clones[self.SelectedClone]["Preset"], self.Clones[self.SelectedClone]["Class"], self.Clones[self.SelectedClone]["Module"], self.Clones[self.SelectedClone]["XP"], self.Clones[self.SelectedClone]["Identity"], self.Clones[self.SelectedClone]["Prestige"], self.Clones[self.SelectedClone]["Name"], limbData)
										if a ~= nil then
				...
				if cont:IsState(Controller.WEAPON_FIRE) then
					if not self.FirePressed then
						...
						if self.SelectedClone > 0 and CF_CountUsedStorageInArray(self.StorageItems) < tonumber(self.GS["Player0VesselStorageCapacity"]) and #self.Clones[self.SelectedClone]["Items"] > 0 then
							...
							CF_PutItemToStorageArray(self.StorageItems, self.Clones[self.SelectedClone]["Items"][self.ClonesInventorySelectedItem]["Preset"], self.Clones[self.SelectedClone]["Items"][self.ClonesInventorySelectedItem]["Class"], self.Clones[self.SelectedClone]["Items"][self.ClonesInventorySelectedItem]["Module"])
						...
						if self.SelectedClone > 0 and #self.StorageFilters[self.StorageControlPanelModes.EVERYTHING] > 0 then
							...
							if #self.Clones[self.SelectedClone]["Items"] < CF_MaxItems then
								if self.StorageItems[itm]["Count"] > 0 then
									...
									self.Clones[self.SelectedClone]["Items"][newitm]["Module"] = self.StorageItems[itm]["Module"]
			...
			if self.ClonesControlMode == self.ClonesControlPanelModes.INVENTORY or self.ClonesControlMode == self.ClonesControlPanelModes.ITEMS then
				...
				if self.SelectedClone ~= nil and self.SelectedClone > 0 then
					-- Print inventory
					local name = (self.Clones[self.SelectedClone]["Prestige"] and self.Clones[self.SelectedClone]["Name"] and self.Clones[self.SelectedClone]["Name"] ~= "") and self.Clones[self.SelectedClone]["Name"] or self.Clones[self.SelectedClone]["Preset"]
					CF_DrawString(name..": "..#self.Clones[self.SelectedClone]["Items"].."/"..CF_MaxItems, pos + Vector(-141 + 12,-60) , 300, 10)
	...
	if self.ClonesInputPos ~= nil and self.ClonesControlPanelActor ~= nil then
		...
		if  count < tonumber(self.GS["Player0VesselClonesCapacity"]) then
			...
			for actor in MovableMan.Actors do
				...
				if actor.GetsHitByMOs and not moving and CF_Dist(actor.Pos, self.ClonesInputPos) <= self.ClonesInputRange and actor.PresetName ~= "Clones Control Panel" and not actor:IsInGroup("Brains") then
					toresettimer = false
					...
					if self.ClonesLastDetectedBodyTime ~= nil then
						...
						if self.Time >= self.ClonesLastDetectedBodyTime + self.ClonesInputDelay and CF_CountUsedClonesInArray(self.Clones) < tonumber(self.GS["Player0VesselClonesCapacity"]) then
							...
							for j = 1, #CF_LimbID do
								self.Clones[c][CF_LimbID[j]] = CF_GetLimbName(actor, j)
							end
							...
							if actor:IsPlayerControlled() then
								self:SwitchToActor(self.ClonesControlPanelActor, controller.Player, CF_PlayerTeam);
							end
