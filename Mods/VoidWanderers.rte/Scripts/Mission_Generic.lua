	...
	if self.Time > self.MissionNextDropShip and #self.MissionLZs > 0 then
		...
		if MovableMan:GetMOIDCount() < CF_MOIDLimit then
			...
			if teamcount[self.MissionCPUTeams[sel]] < 5 then
				...
				if ship then
					for i = 1, count do
						local actor = CF_SpawnAIUnit(self.GS, self.MissionCPUPlayers[sel], self.MissionCPUTeams[sel], nil, math.random() < 0.5 and Actor.AIMODE_SENTRY or Actor.AIMODE_PATROL)
						if actor then
