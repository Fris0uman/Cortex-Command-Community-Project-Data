	...
	for turr = 1, count do
		if MovableMan:IsActor(self.TurretsControlPanelActor[turr]) then
			...
			for plr = 0 , self.PlayerCount - 1 do
				...
				if act and MovableMan:IsActor(act) and act.Pos.X == self.TurretsControlPanelActor[turr].Pos.X and act.Pos.Y == self.TurretsControlPanelActor[turr].Pos.Y and act.Age == self.TurretsControlPanelActor[turr].Age then
					showidle = false
