	...
	for plr = 0 , self.PlayerCount - 1 do
		...
		if act and MovableMan:IsActor(act) and act.PresetName == "Beam Control Panel" then
			...
			for actor in MovableMan.Actors do
				if actor.GetsHitByMOs and self.BeamControlPanelBox:IsWithinBox(actor.Pos) then	-- and actor.HitsMOs
					--actor:FlashWhite(50)
			...
			if cont:IsState(Controller.WEAPON_FIRE) and canbeam then
				if not self.FirePressed then
					...
					for actor in MovableMan.Actors do
						if actor.GetsHitByMOs and actor.PresetName ~= "Brain Case" and (actor.ClassName == "AHuman" or actor.ClassName == "ACrab") then
