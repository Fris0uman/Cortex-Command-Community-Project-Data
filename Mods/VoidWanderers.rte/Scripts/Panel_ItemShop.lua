		...
		if act and MovableMan:IsActor(act) and act.PresetName == "Item Shop Control Panel" then
					...
					if self.ItemShopSelectedItem > 0 then
						...
						if itm ~= nil then
							if self.ItemShopItems[itm]["Type"] == CF_WeaponTypes.BOMB then
								...
								if self.ItemShopItems[itm]["Preset"] and CF_CountUsedStorageInArray(self.StorageItems) < tonumber(self.GS["Player0VesselStorageCapacity"]) and self.ItemShopSelectedItemPrice <= CF_GetPlayerGold(self.GS, 0) then
			...
			for i = self.ItemShopControlItemsListStart, self.ItemShopControlItemsListStart + self.ItemShopControlPanelItemsPerPage - 1 do
				if i <= #self.ItemShopFilters[self.ItemShopControlMode] then
					...
					CF_DrawString((i == self.ItemShopSelectedItem and "> " or "")..(self.ItemShopItems[itm]["Preset"] or "SHIPPING ERROR"), pos + Vector(-130,-26) + Vector(0, (loc) * 12), 90, 10)
			...
			if self.ItemShopControlPanelObject then
				...
				for att in self.ItemShopControlPanelObject.Attachables do
					if att:IsDrawnAfterParent() then
			...
			-- Print manufacturer
			CF_DrawString("Manufacturer: "..(self.ItemShopSelectedItemManufacturer or "Unknown"), pos + Vector(10,-25) , 170, 10)
