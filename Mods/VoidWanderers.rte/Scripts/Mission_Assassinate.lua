...
function VoidWanderers:MissionCreate()
	...
	--if self.MissionDifficulty > 4 and math.random(self.MissionDifficulty) > 4 then
	--	self.MissionBrain = CF_MakeRPGBrain(self.GS, self.MissionTargetPlayer, CF_CPUTeam, cpos, self.MissionDifficulty)
	--else
		...
		if self.SpawnTable == nil then
			if MovableMan:IsActor(self.MissionBrain) then
				if not SceneMan:IsUnseen(self.MissionBrain.Pos.X, self.MissionBrain.Pos.Y, CF_PlayerTeam) then
					self:AddObjectivePoint("KILL", self.MissionBrain.Pos - Vector(0, self.MissionBrain.Radius), CF_PlayerTeam, GameActivity.ARROWDOWN);
				end
			else
