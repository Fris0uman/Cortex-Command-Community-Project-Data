...
function VoidWanderers:MissionUpdate()
	if self.MissionStage == self.MissionStages.ACTIVE then
			...
			if MovableMan:IsActor(self.MissionDevices[i]) then
				...
				if self.MissionDevices[i].Health < 100 and self.MissionAlertTriggered[i] == false then
					...
					for actor in MovableMan.Actors do
						if actor.Team == CF_CPUTeam  and actor.AIMode == Actor.AIMODE_SENTRY and (actor.ClassName == "AHuman" or actor.ClassName == "ACrab") and CF_Dist(self.MissionDevicesPos[i], actor.Pos) < self.MissionAlertRange then
		...
		if self.LastAlertTriggered == false and vats == 1 then
			...
			for actor in MovableMan.Actors do
				if actor.Team == CF_CPUTeam  and actor.AIMode == Actor.AIMODE_SENTRY and (actor.ClassName == "AHuman" or actor.ClassName == "ACrab") then
