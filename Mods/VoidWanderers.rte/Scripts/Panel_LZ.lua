				...
				if actor.Team == CF_PlayerTeam and IsActor(actor) and ToActor(actor).Health > 0 then
					...
					if assignable and actor.PresetName ~= "LZ Control Panel" and (actor.ClassName == "AHuman" or actor.ClassName == "ACrab") then
						...
						for j = 1, #CF_LimbID do
							self.DeployedActors[n][CF_LimbID[j]] = CF_GetLimbName(actor, j)
						end
						--print (#pre)
					end
				end
...
function VoidWanderers:ProcessLZControlPanelUI()
	...
	for i = 1, #self.LZControlPanelActor do
		...
		for plr = 0, self.PlayerCount - 1 do
			...
			if act and MovableMan:IsActor(act) and act.PresetName == "LZ Control Panel" and act.ID == self.LZControlPanelActor[i].ID then
				showidle = false
			end
		end
	...
	for plr = 0, self.PlayerCount - 1 do
		...
		if act and MovableMan:IsActor(act) and act.PresetName == "LZ Control Panel" then
			...
			if self.BombsControlPanelSelectedModes[selectedpanel] == self.BombsControlPanelModes.RETURN then
				...
				for actor in MovableMan.Actors do
					if not self:IsAlly(actor) and (actor.ClassName == "AHuman" or actor.ClassName == "ACrab") and actor.Team ~= Activity.NOTEAM then
						if actor.Team ~= CF_PlayerTeam then
							...
							for i = 1, #self.LZControlPanelPos do
								local dist = SceneMan:ShortestDistance(self.LZControlPanelPos[i], actor.Pos, SceneMan.SceneWrapsX)
								if math.abs(dist.X) < FrameMan.PlayerScreenWidth * 0.5 + actor.Radius and math.abs(dist.Y) < FrameMan.PlayerScreenHeight * 0.5 + actor.Radius then
									inprox = true
									break
								end
							end
				...
				if safe then
					...
					if unsafefriends > 0 then
						local s = unsafefriends > 1 and "S" or "";
	...
	if self.DeployedActors then
		...
		if safe then
			...
			for item in MovableMan.Items do
				if not item.UnPickupable then
					local inprox = false;
					for i = 1, #self.LZControlPanelPos do
						local dist = SceneMan:ShortestDistance(self.LZControlPanelPos[i], item.Pos, SceneMan.SceneWrapsX)
						if math.abs(dist.X) < FrameMan.PlayerScreenWidth * 0.5 and math.abs(dist.Y) < FrameMan.PlayerScreenHeight * 0.5 then
							inprox = true;
							break
						end
					end
					if inprox then
						...
						if count < tonumber(self.GS["Player0VesselStorageCapacity"]) and itemcount < #self.DeployedActors * 4 then
							CF_PutItemToStorageArray(storage, item.PresetName, item.ClassName, item.ModuleName)
							print(item.PresetName .. ", " .. item.ModuleName)
