function Create(self)
	...
	if self.pointed == nil then
		...
		if self.pointed == nil then
			for actor in MovableMan.Actors do
				...
				if dist < 300 and actor.Team ~= self.ownerRoot.Team then
					self.pointed = MovableMan:GetMOFromID(
						SceneMan:CastMORay(
							self.Pos,
							(actor.Pos - self.Pos),
	...
	then
		...
		self.proxCheck = MovableMan:GetMOFromID(
			SceneMan:CastMORay(self.Pos, self.Vel * (2 / 3), self.ID, self.IgnoresWhichTeam, 0, false, 1)
		)
		if self.proxCheck ~= nil then
			if self.proxCheck.RootID == self.pointedRoot.ID then
				self:GibThis()
			end
		end
	else
