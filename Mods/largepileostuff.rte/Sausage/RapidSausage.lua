function Create(self)
	self.skipCheck = 0
	self.frameTimer = 0
	self.whackVel = Vector(10, 0):RadRotate(self.RotAngle + math.rad(180)*math.min(0, self.FlipFactor))
	self.whackDamage = 10
	...
	if self.skipCheck == 0 then
		for actor in MovableMan.Actors do
			if actor.ID == self.owner.RootID and actor:HasObject("Rapid Sausage") == false then
				self.skipCheck = 1
			end
		end
	end
	...
	if self.frameTimer == 3 then
		if self.skipCheck == 1 then
			...
			if self.targetRoot ~= nil then
				...
				local impact = CreateAEmitter("Rapid Impact")
